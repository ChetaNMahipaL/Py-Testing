    ...
    if DEAD_CROW >= 4:
        ...
        for line in lines:
            if VR_INTERIM_POS in line:
                ...
                if index in (1,2):
                    win_flag = all(opponent_1.cr_check(line_point, opponent_1.color) == 1 and
                                   line_point != VR_INTERIM_POS for line_point in line[1:])
                    if not win_flag:
                        break
                else:
                    win_flag = opponent_1.cr_check(line[1],
                                        opponent_1.color) == opponent_1.cr_check(line[2],
                    opponent_1.color) == 1
                    if not win_flag:
                        break
...
class Crow:
    ...
    def cr_movement(self,event_1):
        ...
        if event_1.type == pygame.MOUSEBUTTONDOWN:
            pos = get_coord()
            static = self.cr_check(pos,self.color)
            if static == 1:
                DRAGGING = True
                CR_INTERIM_POS = pos
...
class Vulture:
    ...
    def vr_check(self,coord,color,drag):
        """
        ...
        if result == (-1):
            ...
            if abs(index_1 - index_2 ) == 3:
                ...
                if (index_1 + index_2) % 2 == 0:
                    if opponent_1.cr_check(req_line[int((index_1+index_2)/2)],
                                           opponent_1.color) == 1:
    ...
    def vr_movement(self,event_2):
        ...
        if event_2.type == pygame.MOUSEBUTTONDOWN:
            pos = get_coord()
            static = self.vr_check(pos,self.color,True)
            # print(static)
            if static == 1:
                DRAGGING = True
                VR_INTERIM_POS = pos
        ...
        if event_2.type == pygame.MOUSEBUTTONUP:
            pos = get_coord()
            if DRAGGING and pos != (-1,-1):
                static = self.vr_check(pos,self.color,False)
                # print(static)
                if static == 2:
                    temp = (pos,self.color)
                    pawns.append(temp)
